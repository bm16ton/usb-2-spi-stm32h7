...
static void
usbspi_config (struct usb_interface *dev, int cmd, int value,
	...
	if (inf == 1)
	  {
		  ...
		  retval = usb_find_int_in_endpoint
		  ...
		  if (noirq == 0)
		    {
			    ...
			    priv->int_in_buf =
				    kmalloc (le16_to_cpu
					     (priv->
					      int_in_endpoint->wMaxPacketSize),
					     GFP_KERNEL);
...
*/
		  retval = usb_find_common_endpoints
			  (interface->cur_altsetting, &bulk_in, &bulk_out,
			   NULL, NULL);
		  if (retval)
		    {
